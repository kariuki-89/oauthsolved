...
async function onPageLoad() {
  ...
  if (queryParams.has('code')) {
    console.log('The key "code" exists in the URL parameters.');
    const codeValue = queryParams.get('code');
    const platform=queryParams.get('platform');
    let thedata=await authenticate(codeValue,platform)
    console.log(thedata,"-------566-------")
    let thisUser=localStorage.getItem("uniqueId")
    let uniqueId2=await generateUniqueValue()
    let theConnectedAccount=await getFirestoreItem("connected accounts",platform,thisUser)
    console.log(theConnectedAccount)
    let theDatanotNull=thedata.status=="200 ok"
    let connectedAccountNotNull=theConnectedAccount.status=="true"
    if (theDatanotNull){
      console.log("thedatanotnull")
      if(!connectedAccountNotNull){
        console.log("connectedaccountnotnull")
        let cdnData2={
          uuid: uniqueId2,
          app: platform,
          status: "active",
        ...
        let createdaccount=await cdn_createAgent("connected accounts",cdnData2);
        if(createdaccount.status="200 ok"){
          window.location.href = `/playground`;
        }
        else{
          window.location.href = `/authorizeapps`;
        }
      }
      if (connectedAccountNotNull){
        console.log("connectedId=",theConnectedAccount.id)
        fieldsToUpdate={"token": thedata.Token}
        let updatedNew=await updateDocument("connected accounts", theConnectedAccount.id, fieldsToUpdate)
        if(updatedNew.status=="200 ok"){
          window.location.href = `/playground`;
        } 
        else{
          window.location.href = `/authorizeapps`;
    ...
    console.log('The key "code" does not exist in the URL parameters.');
  }
...
async function generateUniqueValue() {
  ...
  const uniqueNumber = Math.floor(Math.random() * 1e6); // Random number between 0 and 999999
  return `${timestamp}-${uniqueNumber}`; // Combine timestamp and unique number
}
...
const firebaseConfig = {
  apiKey: "AIzaSyCBjqMlaM5honcXDgleu9hwaHZ_PKEjkWY",
  authDomain: "multi-agentbase.firebaseapp.com",
  projectId: "multi-agentbase",
  storageBucket: "multi-agentbase.firebasestorage.app",
  messagingSenderId: "539194652253",
  appId: "1:539194652253:web:e1e58dbd1e33c6506d7fb4",
  measurementId: "G-JLBDBGCLPV"
};
...
document.addEventListener("DOMContentLoaded", async() => {
    ...
    if(linkedinButton){
      ...
      window.location.href = "https://www.linkedin.com/oauth/v2/authorization?response_type=code&client_id=863iuogbciqxja&redirect_uri=http://127.0.0.1:3000/playground.html?platform=app1&state=foobar&scope=w_member_social,openid,profile,email"
      })
    }
    
    if(calendarButton){
      document.querySelector("#calendaroauth").addEventListener("click", async (event) =>{
      window.location.href = "https://accounts.google.com/o/oauth2/v2/auth?client_id=142417176441-2f2g7mqv6fg7sd8avi9agqvt2comleg2.apps.googleusercontent.com&redirect_uri=http://127.0.0.1:3000/playground.html?platform=app2&response_type=code&scope=https://www.googleapis.com/auth/calendar https://www.googleapis.com/auth/calendar.events https://www.googleapis.com/auth/calendar.events https://www.googleapis.com/auth/calendar.events.readonly https://www.googleapis.com/auth/calendar.settings.readonly https://www.googleapis.com/auth/calendar.addons.execute&access_type=offline"
      })
    }

    if(xoauthButton){
      document.querySelector("#Xoauth").addEventListener("click", async (event) =>{
      window.location.href = "https://twitter.com/i/oauth2/authorize?response_type=code&client_id=TVQyTzFpLUF0dGttU1pyaVVFZ2s6MTpjaQ&redirect_uri=http://127.0.0.1:3000/playground.html?platform=app3&scope=tweet.read%20users.read%20offline.access%20tweet.write%20follows.read%20follows.write&state=x&code_challenge=x56fdfdfgh&code_challenge_method=plain"
      })
    }
    ...
    //console.log("tyyyytyyytyt",descriptionInput,nameInput,agentuuid)
      ...
      document.getElementById("theloadingspinner").style.display = "block";
...
const authenticate = async (code,platform) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.open("POST", "http://127.0.0.1:8000/authorize", true);
    ...
    xhr.onreadystatechange = () => {
      if (xhr.readyState === XMLHttpRequest.DONE) {
        if (xhr.status === 200) {
          ...
          reject(new Error(`HTTP error! Status: ${xhr.status}, ${xhr.statusText}`)); // Reject with an error
        }
      }
    };
...
async function cdn_signUp(email, password, name) {
  try {
    ...
    const userSaved = await saveUserToFirestore(user, name);
...
async function cdn_createAgent(table, data) {
  try {
    ...
    lastItemQuery.onSnapshot((snapshot) => {
      ...
      snapshot.forEach((doc) => {
        ...
        if(doc.data().agent==theAgentId&&moveNext==true){
          ...
          const container2=document.getElementById("theloadingspinner")
          
          const container = document.querySelector('#chat-container-message');
          // Scroll to the bottom of the container
          container.scrollTop = container.scrollHeight;
          allMessages.push(theId)
          if(doc.data().agent_type=="received"){
            let thisMess={assistant:message}
            structured_messages.push(thisMess)
          }
        }
        //output.textContent += `${JSON.stringify(doc.data())}\n`;
      });
...
let allApps=[]
appsId=[]
function cdn_listenForSnapshots_connectedaccounts() {
  const user = auth.currentUser;
  ...
  if (user) {
    ...
    itemsCollection.onSnapshot((snapshot) => {
      ...
      snapshot.forEach((doc) => {
        let userSaved=localStorage.getItem("uniqueId")
         console.log(userSaved,"userSaved")
        let thisId=doc.id
        console.log(doc.data().creator,typeof(doc.data()))
        let isThisUser=doc.data().creator==userSaved
        alreadyAdded=!appsId.includes(thisId)
        if(alreadyAdded&&isThisUser){
          let newApp={
            "app":doc.data().app,
      ...
      appsId.push(thisId,)
      console.log("myAPPSS",allApps,isThisUser,alreadyAdded)
        //output.textContent += `${JSON.stringify(doc.data())}\n`;
      });
    }, (error) => {
      console.error("Error in snapshot listener:", error.message,userSaved,doc.data().creator);
    });
  } else {
    console.error("No authenticated user for snapshot listener.");
    //alert("Please sign in to receive snapshots.");
...
async function createWebSocket(quiz) {
    ...
    socket.onopen = function() {
      ...
      if(quiz!=null||quiz!=null){
        ...
        tenlast=structured_messages.slice(-10)
        let payload={query:quiz, history:tenlast,accountdetails:allApps}
        ...
        //data=JSON.parse(event.data)
             ...
             let theData=JSON.parse(event.data)
              ...
              if(theData.status=="finished"){
                document.getElementById("theloadingspinner").style.display = "none";
              }
...
let loader=loadingDiv.appendChild(spinner);

// Append the loading div to the body
return loader
}

async function getFirestoreItem(collection, platform, userId) {
  try {
    const collectionRef = db.collection(collection);

    // Query for the document with a unique id and creatorId
    const querySnapshot = await collectionRef
      .where("app", "==", platform) // Replace "platform" and "platformValue" with your field and value
      .where("creator", "==", userId) // Replace "creatorId" and "creatorUniqueId"
      .get();

    if (!querySnapshot.empty) {
      const documents = [];
      querySnapshot.forEach((doc) => {
        documents.push({ status: "true", Document_data: doc.data(), id: doc.id });
      });
      return documents[0];
    } else {
      return { status: "false", Document_data: "no data" };
    }
  } catch (error) {
    console.error("Error getting data:", error);
    return { status: "false", Document_data: "Error getting data" };
  }
}

async function updateSpecificFields(collectionName, documentId, fieldsToUpdate) {
  try {
